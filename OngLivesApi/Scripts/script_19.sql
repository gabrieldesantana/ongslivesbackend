...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230601163911_AlterVolunt_OngCollums')
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230601163911_AlterVolunt_OngCollums', N'7.0.5');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230601164326_AlterImagemColumn')
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230601164326_AlterImagemColumn', N'7.0.5');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230601170401_FallBackImagem')
BEGIN
    ALTER TABLE [TB_Ongs] DROP CONSTRAINT [FK_TB_Ongs_TB_Imagens_ImagemId];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230601170401_FallBackImagem')
BEGIN
    ALTER TABLE [TB_Voluntarios] DROP CONSTRAINT [FK_TB_Voluntarios_TB_Imagens_ImagemId];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230601170401_FallBackImagem')
BEGIN
    DROP INDEX [IX_TB_Voluntarios_ImagemId] ON [TB_Voluntarios];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230601170401_FallBackImagem')
BEGIN
    DROP INDEX [IX_TB_Ongs_ImagemId] ON [TB_Ongs];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TB_Voluntarios]') AND [c].[name] = N'ImagemId');
    IF @var7 IS NOT NULL EXEC(N'ALTER TABLE [TB_Voluntarios] DROP CONSTRAINT [' + @var7 + '];');
    ALTER TABLE [TB_Voluntarios] DROP COLUMN [ImagemId];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TB_Ongs]') AND [c].[name] = N'ImagemId');
    IF @var8 IS NOT NULL EXEC(N'ALTER TABLE [TB_Ongs] DROP CONSTRAINT [' + @var8 + '];');
    ALTER TABLE [TB_Ongs] DROP COLUMN [ImagemId];
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230601170401_FallBackImagem', N'7.0.5');
END;
GO
